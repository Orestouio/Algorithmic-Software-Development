...
double findTrueDistance(const vector<NeighbourBrute> &bruteForceResults, int id)
{
    for (const auto &nghBrute : bruteForceResults)
    {
        if (nghBrute.id == id)
        {
            return nghBrute.distance;
        }
    }
    return 0; // Indicative of not found
}

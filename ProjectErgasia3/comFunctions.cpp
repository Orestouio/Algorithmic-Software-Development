...
CommandLineArgsCluster parseCommandLineArgsCluster(int argc, char *argv[])
{
    ...
    if (args.configurationFile.size())
    {
        std::ifstream configFile(args.configurationFile);
        std::string line;
...
CommandLinesArgsGraph parseCommandLineArgsGraph(int argc, char *argv[])
{
    CommandLinesArgsGraph args;
    if (argc < 3)
    {
        cout << "Not enough arguments in command line." << endl;
        exit(1);
    ...
    else if (args.m == 2)
    {
        ...
        {
            cout << "The 'I' value should always be greater or equal to 'N' " << endl;
            exit(1);
        }
    }
    else
    {
        cout << "-m should be 1 or 2, any other option is not acceptable! Please try again." << endl;
        exit(1);
    }
}

...
int Hash_Table_LSH::AmplifiedHashFunctionG(ImageVector *key)
{
	...
	{
		unsigned int euc_mod = euclidean_mod(r[i] * h[i], M);
		pre_mod += euc_mod;
	}
	...
	unsigned int g = euclidean_mod(final_mod, this->buckets);
...
void Hash_Table_LSH::FindNeighbours(ImageVector *query)
{
	...
	for (int i = 0; i < vectorSize; i++)
	{
		...
		for (ImageVector *data : table[queryBucket])
		{
			...
			// Store Distances
			query->addNeighbour(data->id, distance, data);
			query->addGraphNeighbour(data);
		}
	}
}
...
vector<ImageVector*> Hash_Table_LSH::LSHGraphnn(ImageVector *v, int N)
{
	...
	{
		cout << "No neighbors found." << endl;
		return vector<ImageVector*>(); // Return an empty vector
	}

	vector<ImageVector*> NearestNeighbours;
	...
	{
		bool exists = false;
		for(ImageVector* n : NearestNeighbours){

			if(n->id == neighbour.img->id){
				exists = true;
				break;
			}
		}
		...
		if(!exists){
			//cout<<"INserting:" << neighbour->id<<endl;
			NearestNeighbours.push_back(neighbour.img);
			c++;
		}
